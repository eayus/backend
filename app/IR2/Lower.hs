...
llExpr localFuncs (Fix e) = case e of
  ...
  R1 (L1 (CallF f xs)) | Just info <- M.lookup f localFuncs -> do
    ...
    pure $ Fix $ L1 $ CallF info.newName (map (Fix . R1 . L1 . uncurry Var) info.extraParams ++ xs')
...
exprCaptures locals (Fix e) = case e of
  R1 (R1 (L1 (Var v a))) -> M.singleton v a
  R1 (R1 (R1 (Let v x y))) -> exprCaptures locals x `M.union` M.delete v (exprCaptures locals y)
  R1 (R1 (R1 (Match x cs))) -> exprCaptures locals x `M.union` M.unions (map (clauseCaptures locals) cs)
